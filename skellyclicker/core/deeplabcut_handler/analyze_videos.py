...
def analyze_videos(config_path: str | Path, 
                   path_to_recording_folder: str | Path | None = None,
                   output_folder: str | Path | None = None, 
                   annotate_videos: bool = False):
    config = auxiliaryfunctions.read_config(config_path)
    if path_to_recording_folder is None:
        path_to_recording_folder = config.get("skellyclicker_folder_of_videos", Path(config_path).parent)
        print(f"Using default path to recording folder: {path_to_recording_folder}")
    if output_folder is None:
        output_folder = Path(config.get("project_path", Path(config_path).parent)) / "model_outputs" / f"model_outputs_iteration_{config['iteration']}_{Path(path_to_recording_folder).name}"
        print(f"Using default output folder: {output_folder}")
    Path(output_folder).mkdir(parents=True, exist_ok=True)

    print(f"Analyzing videos in {path_to_recording_folder}, saving to {output_folder}")
    ...
    if annotate_videos:
        ...
        # TODO: replace with skellyclicker based video annotation
        deeplabcut.create_labeled_video(config=str(config_path), videos=str(path_to_recording_folder), videotype=".mp4", destfolder=str(path_to_recording_folder))
    else:
        print("Skipping video annotation")

    merge_csvs_for_skellyclicker(path_to_recording_folder, Path(output_folder) / f"skellyclicker_machine_labels_iteration_{config['iteration']}.csv")
